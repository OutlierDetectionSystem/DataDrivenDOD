...
public class DistributeCalSupportCellsOPT {
	...
	public static class DistributedSupportCellReducer extends Reducer<IntWritable, Text, NullWritable, Text> {
		...
		public void dealEachPartition(int indexOfPartition) {
			...
			int supportCellsSize;
			supportCellsSize = (int) (Math.ceil(radius / smallRange));

			int[] newindexes = new int[num_dims * 2];
			for (int i = 0; i < num_dims; i++) {
				newindexes[2 * i] = Math.max(0, indexes[2 * i] - supportCellsSize);
				newindexes[2 * i + 1] = Math.min(cell_num, indexes[2 * i + 1] + supportCellsSize);
